...
function IRC( options ) {
  ...
  {
    var that = this, i, start = 0, buffer2, buffer = []
    ...
    for (i = 0; i < data.length; i++)
    {
        ...
        {
            buffer2 = new Buffer(i - start)
            data.copy(buffer2, 0, start, i + 1)
            start = i + 2
            buffer.push(buffer2)
        }
    }
    
    buffer.forEach(function(item) { parseMessage.call(that, item)} )
  }    
  ...
  function parseMessage( data ) { var buffer, last, message, command, i
    ...
    // Compile
    try {
      message = Compiler.compile( data + "\r\n" )
    }
    catch ( err ) {
      Log.error( "[ERROR] Failed parsing '" + data + "'" )
      if ( this.options.die )
      {
        throw err
      }
    ...
    // Emit event
    if (emitter.listeners(message.command).length > 0)
    {
        emitter.emit( message.command, message )
    }
    else
    {
        emitter.emit( 'catch_all', message )
    }
  }
  ...
  **/
  this.connect = function( hollaback ) {
    ...
    if ( !stream || ( not_open = ( [ 'open', 'opening' ].indexOf( stream.readyState ) < 0 ) ) ) {
      ...
      // Receive data
      stream.on( 'data', receiveMessage.bind( this ) )
  ...
  **/
  this.once = function( event, hollaback ) {
    ...
    while (typeof(internal.single_listener_cache[hollaback+'-'+add]) != 'undefined')
    {
        add++
    }
    ...
    internal.single_listener_cache[index] = function( hollaback, message ) {
      // Call function
      ...
      setTimeout( function() {
        ...
        if (internal.single_listener_cache[index])
        {
            emitter.removeListener( event, internal.single_listener_cache[index] )
            delete internal.single_listener_cache[index]
        }
      }, 0 )
    }.bind( this, hollaback )
    
    // Listen
    emitter.addListener( event, internal.single_listener_cache[index] )
  }
